...
class SecurityTester {
  ...
  private results: Array<{ test: string; success: boolean; message: string; details?: any }> = [];

  constructor() {
    this.setupTests();
  }

  private addTest(name: string, description: string, test: SecurityTest['test']) {
    this.tests.push({ name, description, test });
  }

  private async runTest(test: SecurityTest) {
    console.log(`\n🧪 ${test.name}`);
  ...
  private setupTests() {
    ...
    this.addTest(
      ...
      this.testAdminProtection.bind(this)
    );

    // Input Validation Tests
    this.addTest(
      'SQL Injection Prevention',
      'Tests that SQL injection attempts are blocked',
      this.testSQLInjection.bind(this)
    );

    this.addTest(
      'XSS Prevention',
      'Tests that XSS attempts are blocked by CSP',
    ...
    this.addTest(
      ...
      this.testPasswordProtection.bind(this)
    );

    // File Upload Tests
    this.addTest(
      'File Upload Validation',
      'Tests that file uploads are properly validated',
      this.testFileUploadValidation.bind(this)
    );
    ...
    this.addTest(
      ...
      this.testErrorDisclosure.bind(this)
    );

    // HTTPS Tests (if applicable)
    this.addTest(
      'Security Headers Present',
      'Tests that all security headers are present',
  ...
  private async testAdminProtection() {
  ...
  private async testPasswordProtection() {
  ...
  private async testErrorDisclosure() {
